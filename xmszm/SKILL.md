---
name: xmszm
description: 仅在写代码任务中触发。执行项目定位、代码搜索、实施与结果记录的标准流程。支持多项目环境与多人协作。
---
...
| 需要修改/开发/修复代码 | 代码开发 | `guides/CODE-WORKFLOW.md` | 默认工作流，支持快速模式 |
| 既修改代码又需记录 | 代码开发 | `guides/CODE-WORKFLOW.md` | 默认工作流，Step 5记录 |
...
当同一输入同时出现多类关键词时，按以下优先级判定：  
1. 含“修改/修复/新增/删除/重构”等代码动作词 → **代码开发**  
2. 仅含“查看历史/统计报告”等查询词 → **日志操作**  
3. 同时含“修改+记录” → **代码开发**（在 Step 5 记录）  

### 术语定义（必须区分）

1. **初始化**：仅指创建/修复 `.xmszm` 归档结构。  
2. **补记录**：将本次改动写入 Step 5（可写入归档，或输出结构化摘要）。  
3. 未记录改动不等于初始化，归类为“补记录”。  
...
5. **默认一次跑完**：默认静默执行主流程，不做中途频繁询问
6. **仅高风险询问**：删除/跨项目写入/权限异常时再询问
7. **初始化边界**：初始化只允许修改 `.xmszm` 与 `.gitignore`
8. **归档可选**：`.xmszm` 不存在时不阻断任务，Step 5 输出结构化变更摘要
9. **Git 可选**：无 Git 时禁止依赖 `git diff`，改为手动信息收集
...
1. **小改快速交付**：`定 → 做 → 记`（Fast Track）  
2. **常规功能修改**：`InitHook → 定 → 查 → 搜 → 做 → 记`  
3. **多项目未指明**：`定（交互选项目）→ 后续流程`  
4. **仅查历史**：路由到 `LOG-SKILL`，执行查询流  
5. **仅做统计**：路由到 `LOG-SKILL`，执行统计流  
6. **无 Git 环境**：日志记录走手动输入模式  
7. **并发写日志**：追加写入，冲突按 Git 人工合并  
8. **归档缺失/损坏**：InitHook 自动静默补全后继续执行  
